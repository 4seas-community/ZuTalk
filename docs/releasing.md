...
- **发出去之后**：逐个 HEAD appcast 里的地址，并确认稳定 appcast 地址
  已经指向新版本。
- **包里不许有构建机的痕迹**：发布构建开启 deployment postprocessing 并
  在签名前剥离调试符号。曾经有 518 处 `/Users/<用户名>/...` 躺在主二进
  制的调试信息里跟着每个安装包发出去；家目录名往往就是一个真名。
  隐私探针不再经 `strings`——macOS 的 `strings` 对这个上百 MB 的胖二进
  制一个字符串都吐不出来（`-a`、`-arch all` 都是 0），于是「无本机路径」
  这句话曾经是靠一个瞎了的探针说出来的。现在直接二进制安全 grep，并把
  命中内容打出来。

### 删过旧 Release 之后

历史 Release 被删掉之后，`build/dmg/` 里留着的旧 DMG 就不再对应任何已
发布的版本。下一次发布会在「核对 delta 基线」那一步被挡下——那是断言在
做它该做的事，不是故障。把不再在线的旧 DMG 从 `build/dmg/` 移走即可，
基线随之只剩仍然在线的版本，全量下载对所有人始终可用。
