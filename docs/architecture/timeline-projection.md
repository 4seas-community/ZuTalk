...
## 8. 上线之后：一次由用户症状发起的回访（0.4.0）

用户的原话是两句：「有时候语言不在同一行，会有空格、间隔这些格式上的问题」，
以及「录了一段时间之后，翻译就会变得很卡」。按 §6 的回路走一遍，得到的东西
比症状本身多——包括两条被实测证伪的流行解释，和一个本设计自己引入的缺陷。

### 8.1 「越录越卡」不在翻译上

翻译与转录共用同一条 Soniox 连接，`context` 在建连时一次性绑定，**不随时长
增长**；仓库里没有 LLM 翻译器。慢的是实时块投影：它在每个 capture event 上
重放整场 Final 事实，而重放里每条 utterance 各自读一次整份文档、各自提交一次。
`get_state`/`set_state` 每次都是整份 JSON 深拷贝 + 整份 schema 校验 + 整份
diff，于是单次投影是 O(n²)。

实测（release，稳态=已有 n 行再来一条 Final）：

| 行数 | 修前单次投影 | 修后 load+upsert | 修后整条 `project_notebook_realtime_incremental` |
|---|---|---|---|
| 100 | 140 ms | 1.0 ms | 8.6 ms |
| 400 | 2492 ms | 3.1 ms | 27.6 ms |
| 800 | 8616 ms | 6.1 ms | 46.3 ms |

**关键观察：这些工作全部是白做的。** 连做 5 轮同样的重放，导出快照字节数逐轮
完全相同——内容零变化，成本照付。

改法把「重放」和「写」分开:

- 重放仍是整场。它是幂等性与崩溃恢复的依据（「跑第二遍」就是恢复路径），
  不能换成增量；
- 写不再是。文档已经这么说的块直接跳过，块列表整次只读一次、批内效果在本地
  模型上模拟，剩下的经 `TranscriptProjection::machine_upsert_blocks` 一次提交。

回归测试钉的是**写入块数**而不是墙钟：重放一份文档已持有的快照必须写 0 块。
墙钟在负载下不可靠，块数不会。

### 8.2 撤回一条推测行曾要求重发整场

`removed_sequences` 从来没有过 FFI 形态，于是「某行消失了」唯一的告知方式是
整场重发。而 provider 替换推测尾部是常态事件，那次整场读又发生在**握着回调
邮箱锁**时——邮箱正是所有字幕 delta 的发布边界。

实测 800 行：整场读 10.4 ms，delta 读 0.05 ms，相差 200 倍，且每次全量事件读
两遍。现在撤回随 delta 走，`requires_full_snapshot` 整条退役。

一条契约值得写下来：**同一个序号不会同时出现在「新增」与「删除」两个列表里**。
撤回一条 partial 有时会留下一个只有译文的壳——那是新增，不是删除。发布端因此
做了过滤，接收端可以任意顺序应用两个列表。

### 8.3 索引按步走，不按句走

搜索索引是每 session 一行的 replace-in-full，重建要把整场文本重拼一遍。按句
重建后来成了这条链路上最贵的一段，而它买不到任何人能感知的东西：录制中用户在
录制，转录界面直接读事实层而不读索引，没有人能比一句话被说出来更快地搜索它。

所以索引对**进行中的**会话按步跟随，对**已完成的**会话精确：定稿必然重建，
录制中按水位间隔重建，崩溃由启动修复兜底。

### 8.4 格式：四个成因，其中一个不是缺陷

1. **源文车道不 trim，译文车道 trim。** Soniox 把词间空格放在 token 上
   （`" world"`），所以中途开始的段落自带一个前导空格。事实层保留它是对的；
   问题是只有译文车道在下游被 trim 了。现在 trim 发生在 **FFI 边界**——事实层
   与一切渲染它的东西之间的那道缝——一次覆盖 App、分享页、网页与 P2P 接收端。
2. **段间拼接曾与文字系统无关。** 两个中文段之间被塞进 ASCII 空格。规则改为
   常规排版规则：两个密排字符之间不加；密排与分词文字之间加一个（`他说ZuTalk`
   挤在一起同样是错的）；任一侧是全角标点则不加，`。` 自带半个字身的空隙。
   韩文写空格，Hangul 码位刻意落在密排区间之外，因此自动落在正确的一侧。
3. **栏位在窄画布上从三栏直接塌成一栏。** 内边距与栏间距从来没被算进容量。
   现在逐栏降级（3→2→1），chrome 由布局策略统一扣除，自动字号预留同一份。
4. **观众画布不做行对齐**——这是 §3 的取舍，不是缺陷。但有一个真缺陷藏在它
   旁边：等待省略号作为堆叠兄弟排在卡片之后，把该列最新的一句从共享底边顶了
   上去。底边对齐是这个布局唯一承诺的对应关系，一个「慢半拍」的车道因此看起来
   就是「和别的语言错开了」。省略号是承诺不是内容，改为浮层，不再位移任何东西。

### 8.5 画布不出现系统话术，这条是硬的

会话模式曾把语言未定的行渲染成带图标的状态行（"语言待定"）。它同时违反两件事：
观众画布不变量第 4 条（永不出现系统话术），以及它自己的形状——一条被标签化的行
和周围的行长得不一样，一条未识别的行就足以在视觉上打断栏格。

现在那些行在**画布上**就是话：整宽、无标签、无图标。操作者仍然需要知道，那属于
操作者 chrome。**持久转录页保持带标签的严格处理**：它的读者不在现场、事后阅读，
知道「身份从未确认」对他有用。同一个事实在两个界面两种呈现，这是有序不变量的
直接推论，不是不一致。

### 8.6 改拼接规则会碰老数据（这次踩到了）

老版本存下的中文/泰文车道里带着旧规则拼出的空格。新规则重新合成同一批段落
会得到「同样的字、少一个空格」，不再是字节前缀。而启动恢复在寻找新绑定**之前**
会先重放全部已绑定的行，所以每次启动都会走一遍这条路。

第一版补丁只放宽到「忽略空白的前缀**且更长**」，不够——重放没有新增，长度不增，
`Conflict` 掀掉整个 reconcile 事务，待补绑的段落**永远绑不上，且错误被上游吞掉**。
最终判据是 `preserves_settled_lane`：新文本必须仍然包含旧文本说过的一切、顺序
不变、不收回任何内容——忽略空白时接受等长。

同一判据也用在共享文档的「用户接管」判定上。那里原先只看「与机器现在要写的
不一致」，于是升级后会把机器自己的增长误判成人工订正，把车道永久让给一个并不
存在的用户。真的人工订正会岔开，不会是前缀。

教训写进 §6 的回路：**「合成只在有新段落时发生」是读代码推断出来的，是错的。**
端到端测试第一次运行就失败，才把它暴露出来。
