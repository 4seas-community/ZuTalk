...
**完整形态与压扁行并存（2026-08-09 起）。** 初版实现曾把预览帧拍成只剩
「原文 + 单个译文」的行列表,多语言 lane、cue、lane 健康、时间锚全部丢失,
接收端因此还原不出主播画布的观感。现在 `CaptionFrame` 同时携带:压扁的
`lines`（旧接收端的兼容投影,`vt-share` 不依赖 `vt-ffi`,故完整形态是逐字段
镜像声明）与 `utterances` / `cues` / `lane_health` / `session_id`（serde
default,旧帧缺席不炸）。两份由广播端从同一帧翻出,不存在分歧源;接收端经
`share_state().remote_preview` 还原成 `FfiNotebookCaptureLivePreview`,
主机本地的投影水位与排队诊断不过网。

**观看端的字幕画布与本机录音是同一块（2026-08-09 起）。** 完整形态过了网,
观看端却仍走一条独立的渲染:一列滚动的横条,原文在上译文在下,既不分栏
也不认语言 —— 同一场三语会议,主播看到三栏,进房间的人看到一列,观众得
重新学一次怎么读。现在 `SubtitleOverlayView` 的画布只认一个输入值
（`AudienceCanvasInput`：语言、句子、归栏函数、按语言的 cue、坏掉的车道）,
本机采集与远端帧各自填它,渲染一份。

栏目从帧里认,因为观看的人没有主播的采集档案（不曾配置主播讲什么、译
什么）：取 **主播真的在跑的车道**（lane health / cue 的目标语言）加上这一帧
的主导原文语言。认车道而不认「出现过的语言」，是因为语言识别会飘——一句
被误判成法语的中文不该凭空长出一栏法语，飘出来的句子落进画布本来就有的
「没有归属」条，与主播本机的处置一致。两方对谈的主播不发 cue、译文绑在
句子上，那份绑定按 cue 的形状原样搬过来（不是重做对应关系，红线仍在）。
...
**现状（2026-08-08）**：以上三条已落地，形态与两处澄清——

- 指示器在三个表面：采集工具条（完整版，带「这段不共享」/「恢复」）、菜单栏
  录音 popover（同完整版——App 缩在后台录音时这是唯一可见表面）、全屏 REC
  药丸（被动琥珀电波，只在真的在播时亮；药丸 ignoresMouseEvents，不给按钮）。
  指示器的判定与 `ShareCaptionTap::broadcast` 的放行逻辑同源
  （`session_broadcast_status`），不存在恒真指示器。
- 「一键可关」= per-session 静音清单（进程内，随停止共享清零）。静音只扣下
  这一段录音的字幕，共享本身照旧。
- 「记住」目前是 **UI 层的确认记忆**（UserDefaults 里的已确认 Notebook 集合）：
  确认过的 Notebook 下次开共享不再弹确认框。共享状态本身不持久——停止或
  退出后不会自动恢复房间。单条录音范围每次都确认（它交出的是对方保留的
  副本，分量更重）。
- 顺带修掉的一个洞：Notebook 范围的广播以前**不按 Notebook 过滤**——共享着
  A 本、去 B 本录音，字幕照发。现在 tap 在采集启动处绑定录音的 notebook
  归属，范围不符直接丢帧。
...
- **范围判定（观看端）**：只认已入册的 id——Session 范围在加入时由
  分享码钉死那一个;Notebook 范围由**主播在字幕通道宣告过的
  session** 逐个入册（2026-08-09 起,`share_state` 吸收帧时登记）。
  字幕帧只来自与主播 QUIC 认证的直连、且已过房间范围检查,帧里的
  session id 等价于**主播自报**——与分享码钉死同一信任级。
  **成员自报的 id 仍然一律拒收**:那才是 bridge 键位抢占面
  （远端内容冒名顶替本机同 id 文档）的入口,宿主签名的文档清单
  成形之前不冒这个险。挂载处另有占用防御:id 已被本机其它文档
  占用时拒绝挂载共享文档;
...
端到端叙事测试(`vt-ffi/tests/share_ux_end_to_end.rs`)按用户实际经历
走通整场后,暴露并修掉了两处链路断裂。两条都升格为本节的设计约束:

- **发布水位必须跳过经 apply 合入的远端 ops。** 收端文档的发布水位若从
  空起步,观看端第一笔订正会把宿主的建块历史整包导出;对端准入门
  (成员不得增删块)整笔拒收,表现为「订正永远到不了主持人」且两边都
  不报错。`SharedDocSync::apply` 在合入后把导入前后版本向量的差并进
  发布水位——房间给我的内容不由我再发回房间。
- **DocSyncContext 的名册必须随房间在场进化。** enable 时的名册只是
  快照;宿主先 enable、观看端后进来,冻结的快照会让每一个后来者的
  订正死在成员检查上。房间事件循环在名册变化时把在场名册刷进
  DocSyncContext,enable 时也以房间实时在场为准。§10 的「名册怎么
  分发与更新」由此有了 v1 答案:主持人 gossip 广播 + 本机镜像进
  准入上下文;**签名名册仍是未决项**。
- **成员更新由主持人转发(星型)。** 文档更新走成对直连流
  (成员↔主持人),成员之间没有连接 —— 不转发,A 的订正到了主持人就
  停下,B 只有退房重进(催缺)才看得到,三人协同实为三场两人协同。
  主持人把**过完整条准入链**的成员信封原样转发给所有文档连接:签名
  仍是原作者的(接收端照常按名册验 A,不是主持人替 A 背书),被拒的
  更新不扩散;只有主持人转发,观看端收到自己的回声就地止步,不成环。

收件台账的补全:每条带收到时间(文件 mtime,最新在前)、可删除
(只删本机副本;当前房间范围内的那条禁删——doc-sync 会按需重建,
「删了又回来」比禁用更坏)。只读锁从全局开关改为**只挂当前房间
范围的那一条**:只读是房间的属性,不是收件的属性。
