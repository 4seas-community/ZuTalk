...
//! language set, decrypts at most `ZUTALK_MEASURE_SECONDS` into memory, and
...
fn data_dir() -> PathBuf {
    std::env::var_os("ZUTALK_DATA_DIR")
        .map(PathBuf::from)
        .expect("ZUTALK_DATA_DIR must explicitly identify the approved local profile")
}

fn requested_seconds() -> u64 {
    std::env::var("ZUTALK_MEASURE_SECONDS")
        .ok()
...
fn select_capture(conn: &Connection) -> CaptureCandidate {
    ...
    .expect("no completed retained ZuTalk capture is available")
}
...
async fn measures_saved_capture_across_three_translation_targets_without_content_output() {
    ...
    .expect("open ZuTalk database read-only");
    ...
    let canonical_config = SttConfig {
        ...
        client_reference_id: Some(format!("zutalk-saved-latency-{run_nonce}-canonical")),
        ..Default::default()
    };
        ...
        .map(|target| {
            let config = SttConfig {
                ...
                client_reference_id: Some(format!(
                    "zutalk-saved-latency-{run_nonce}-target-{target}"
                )),
                ..Default::default()
            };
            (
