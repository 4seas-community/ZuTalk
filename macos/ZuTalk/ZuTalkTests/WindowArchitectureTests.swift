...
final class WindowArchitectureTests: XCTestCase {
    ...
    func testMainWindowMetrics_sanitizeLegacyAutosavedFrameRemovesPersistedValue() {
        let suiteName = "ZuTalkTests.MainWindowMetrics.\(UUID().uuidString)"
    ...
    func testDynamicIslandSources_areRemovedAfterMenuBarCutover() {
        let zutalk = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk", isDirectory: true)
        ...
        for path in mustNotExist {
            let url = zutalk.appendingPathComponent(path)
            XCTAssertFalse(
    ...
    /// ZuTalkApp.swift must boot the menu-bar coordinator, not the island.
    func testZuTalkApp_installsMenuBarCoordinatorInsteadOfDynamicIsland() throws {
        let appSource = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/ZuTalkApp.swift")
    ...
    func testWindowSurfaceID_doesNotIncludeDynamicIsland() throws {
        let source = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/WindowSystem/WindowSurfaceID.swift")
    ...
    func testWindowCoordinator_doesNotOwnSubtitleTogglePolicy() throws {
        let coordinatorSource = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/WindowSystem/WindowCoordinator.swift")
    ...
    func testWindowCommandRouter_routesSubtitleCommandThroughSubtitleCoordinator() throws {
        let routerSource = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/WindowSystem/WindowCommandRouter.swift")
    ...
    func testZuTalkApp_doesNotControlSubtitleOverlayWhenCaptureChanges() throws {
        let appSource = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/ZuTalkApp.swift")
    ...
    func testMainShellView_ownsRealMainWindowContentInsteadOfPlaceholderBanner() throws {
        ...
        XCTAssertFalse(contents.contains("Text(\"ZuTalk V2\")"))
    ...
    func testMainShellNamesNotebookRoutesAndDoesNotShowFakeIdentity() throws {
        ...
        XCTAssertTrue(contents.contains("softwareUpdate.activity"))
        XCTAssertTrue(contents.contains("updates.downloading"))
    ...
    func testZuTalkApp_keepsMainWindowOutsideAppEntryPoint() throws {
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk", isDirectory: true)
