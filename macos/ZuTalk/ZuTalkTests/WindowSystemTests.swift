...
private final class NotebookCoreAvailability {
    var core: (any ZuTalkCoreProtocol)?
}
...
final class WindowSystemTests: XCTestCase {
    ...
    func testMainNavigationStore_retriesLaunchRestoreAfterCoreBecomesAvailable() throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-retry-launch-notebook-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tempDir)
        defer {
            try? core.shutdown()
            try? FileManager.default.removeItem(atPath: tempDir)
    ...
    func testMainNavigationStore_staleNotebookFallsBackToAnAvailableNotebook() throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-stale-notebook-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tempDir)
        defer {
            try? core.shutdown()
            try? FileManager.default.removeItem(atPath: tempDir)
    ...
    func testGlobalCaptureRouteReturnsToCaptureNotebookAfterUserBrowsesAnotherNotebook() throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-capture-route-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tempDir)
        defer {
            try? core.shutdown()
            try? FileManager.default.removeItem(atPath: tempDir)
    ...
    func testKnowledgeProfileStore_hasNoPlaintextSidecarAuthority() throws {
        let sourceURL = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("ZuTalk/Pages/KnowledgeLibraryPage.swift")
    ...
    func testSubtitleOverlayBackdropIsTranslucentWithoutBackdropBlur() throws {
        ...
        let sourceURL = URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent(
                "ZuTalk/WindowSystem/Surfaces/SubtitleOverlayController.swift"
            )
        let source = try String(contentsOf: sourceURL, encoding: .utf8)
