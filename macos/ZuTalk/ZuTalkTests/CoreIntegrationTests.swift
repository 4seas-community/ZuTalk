...
final class CoreIntegrationTests: XCTestCase {
    ...
    func testCoreInit() throws {
        let tmpDir = NSTemporaryDirectory()
            .appending("zutalk-test-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tmpDir)
    ...
    func testUnitTestDefaultDataDirIsOutsideRealApplicationSupport() {
        ...
        XCTAssertFalse(dataURL.path.contains("Library/Application Support/ZuTalk"))
    }
