...
final class IntegrityChecksTests: XCTestCase {
    ...
    func testCheckInfoPlistMicrophoneUsageDescription_passesInTestBundle() {
        // unit test 跑在 ZuTalk.app host 进程内, Bundle.main 是 ZuTalk.app
        // ZuTalk.app 应当包含 NSMicrophoneUsageDescription。
        let result = IntegrityChecks.checkInfoPlistMicrophoneUsageDescription()
        XCTAssertTrue(
            result.passed,
            "ZuTalk.app must have NSMicrophoneUsageDescription, got: \(result.detail ?? "")"
        )
    }
    ...
    func testCheckCoreClientInitialized_inTestEnvironment() {
        // CoreClient 在 test setup 时会被初始化 (各 test class setUp 调 ZuTalkCore.new)
    ...
    func testCheckAllZuTalkNotificationsHaveAtLeastOneObserver_namesValid() {
        let result = IntegrityChecks.checkAllZuTalkNotificationsHaveAtLeastOneObserver()
