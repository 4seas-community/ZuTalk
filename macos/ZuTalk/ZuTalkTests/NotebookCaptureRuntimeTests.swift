...
final class NotebookCaptureRuntimeTests: XCTestCase {
    ...
    func testTranscriptClipboardPublishesOnlyExplicitNonEmptyText() {
        let pasteboard = NSPasteboard(
            name: NSPasteboard.Name("xyz.voice.zutalk.tests.\(UUID().uuidString)")
        )
    ...
    func testRealtimeRouteBindsNewSessionImmediatelyButManualNotesStaySelected() async throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-start-route-\(UUID().uuidString)")
    ...
    func testCaptureStartAlwaysRevealsRealtimeCommandCenter() async throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-manual-start-route-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tempDir)
        defer {
            try? core.shutdown()
            try? FileManager.default.removeItem(atPath: tempDir)
    ...
    func testStartingFromSettingsAlwaysRevealsTheNewRealtimeSession() async throws {
        let tempDir = NSTemporaryDirectory()
            .appending("zutalk-settings-start-route-\(UUID().uuidString)")
        let core = try ZuTalkCore.newDeferred(dataDir: tempDir)
        defer {
            try? core.shutdown()
            try? FileManager.default.removeItem(atPath: tempDir)
    ...
    func testMicrophoneTapClosureOnlyPublishesToPreallocatedRing() throws {
        let source = try String(
            contentsOf: URL(fileURLWithPath: #filePath)
                ...
                .deletingLastPathComponent()
                .appendingPathComponent("ZuTalk/Capture/MicrophoneCapture.swift"),
            encoding: .utf8
        )
    ...
    func testKnowledgeLibraryImportsPackJSONAndPersistsEditsThroughRustBoundary() throws {
        ...
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("knowledge-import-\(UUID().uuidString).zutalk-pack.json")
        ...
        XCTAssertEqual(replacementObject["schema"] as? String, "zutalk.context-pack.v1")
...
private final class FakeNotebookCaptureClient: NotebookCaptureClienting {
    ...
    var previewSerializedContext = "{\"terms\":[\"ZuTalk\"]}"
    ...
    func createLibraryContextPack(title: String) throws -> NotebookContextPackDTO {
        ...
        libraryDocuments[id] = "{\"schema\":\"zutalk.context-pack.v1\",\"title\":\"\(title)\",\"sources\":[]}"
        return pack
    }
