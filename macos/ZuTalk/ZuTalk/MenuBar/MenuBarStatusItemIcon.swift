...
enum MenuBarStatusItemIcon {
    ...
    /// The ZuTalk mark is a template image so macOS handles light/dark
    /// menu-bar contrast.
    static let idle: NSImage = {
        let image = NSImage(named: "ZuTalkMark")
            ?? NSImage(systemSymbolName: "waveform", accessibilityDescription: "ZuTalk")
            ?? NSImage()
