...
@main
struct ZuTalkApp: App {
    @NSApplicationDelegateAdaptor(ZuTalkAppDelegate.self) private var appDelegate

    init() {
        // 必须在任何人碰数据目录或偏好设置之前 —— 包括下面这行崩溃诊断。
...
/// - 注册全局快捷键（⌃⌥V / ⌃⌥R / ⌃⌥L 等）
@MainActor
final class ZuTalkAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if let initError = CoreClient.shared.initError, !TestEnvironment.isAnyTestMode {
                DebugLog.error("ZuTalk core unavailable", detail: initError)
            }
