...
//   - 所有 info/warn/error 都追加到 ~/Library/Application Support/ZuTalk/debug.log
//
// 用法:
...
/// @AppStorage 存的 key
enum DebugModeKey {
    static let enabled = "zutalk.debugMode"
}
...
enum DebugLog {
    ...
    /// `~/Library/Application Support/ZuTalk/debug.log`
    static var logFileURL: URL {
        let fm = FileManager.default
        let support = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSTemporaryDirectory())
        let dir = support.appendingPathComponent("ZuTalk", isDirectory: true)
    ...
    private static let writeQueue = DispatchQueue(label: "xyz.voice.zutalk.debug-log")
