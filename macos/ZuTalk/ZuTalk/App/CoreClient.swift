...
/// 在 App 启动时初始化，初始化路径：~/Library/Application Support/ZuTalk
@MainActor
final class CoreClient {
    ...
    private init() {
        ...
        do {
            ...
            let core = try ZuTalkCore.newDeferred(dataDir: dataDir)
    ...
    /// 默认数据目录：~/Library/Application Support/ZuTalk
        ...
        if TestEnvironment.isUnitTestMode || TestEnvironment.isUITestMode {
            if let explicitPath = environment["VT_TEST_DATA_DIR"], !explicitPath.isEmpty {
                ...
                dir = fm.temporaryDirectory
                    .appendingPathComponent("ZuTalkTests", isDirectory: true)
                    .appendingPathComponent(String(ProcessInfo.processInfo.processIdentifier), isDirectory: true)
            }
        } else {
            let appSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
                ?? URL(fileURLWithPath: NSTemporaryDirectory())
            dir = appSupport.appendingPathComponent("ZuTalk", isDirectory: true)
        }
