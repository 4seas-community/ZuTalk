...
enum IntegrityChecks {
    ...
    private static let logger = Logger(subsystem: "xyz.voice.zutalk", category: "integrity")
    ...
    static func runAll() -> [IntegrityCheckResult] {
        ...
        results.append(checkAllZuTalkNotificationsHaveAtLeastOneObserver())
    ...
    /// 调用时机: ZuTalkAppDelegate.applicationDidFinishLaunching 末尾
    @MainActor
    ...
    /// ZuTalkCore (UniFFI Rust handle) 必须初始化
    @MainActor
    ...
    static func checkAllZuTalkNotificationsHaveAtLeastOneObserver() -> IntegrityCheckResult {
        ...
        if raw.contains(where: { $0.isEmpty }) {
            return .fail("ZuTalk notifications", "found empty notification name")
        }
        if Set(raw).count != raw.count {
            return .fail("ZuTalk notifications", "found duplicate notification names")
        }
        return .ok("ZuTalk notifications (\(names.count) names)")
    }
    ...
    static func checkApplicationSupportDirWritable() -> IntegrityCheckResult {
        ...
        let zutalkDir = dir.appendingPathComponent("ZuTalk", isDirectory: true)
        try? FileManager.default.createDirectory(at: zutalkDir, withIntermediateDirectories: true)
        if !FileManager.default.isWritableFile(atPath: zutalkDir.path) {
            return .fail("Application Support", "ZuTalk dir not writable: \(zutalkDir.path)")
        }
        return .ok("Application Support (\(zutalkDir.path))")
    }
...
// MainWindowOpener.isRegistered 在 ZuTalkApp.swift 里直接定义
